...
export default function Footer({ lang }: { lang: string }) {
  ...
  return (
    <footer className="w-full bg-surface text-white px-6 md:px-20 pt-24 pb-10 border-t border-white/10">
      {/* TÍTULO */}
      <div className="flex justify-between items-start md:items-center mb-16">
        <h2 className="text-4xl md:text-6xl font-medium uppercase tracking-tight">
          {t.footer.title}
        </h2>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-20 mb-24">
        ...
        <div className="flex flex-col space-y-4 text-3xl md:text-4xl font-light">
          <Link href={`/${lang}/projects`} className="hover:opacity-80">
            {t.footer.links.projects}
          </Link>
          {/* Si tienes página de servicios: */}
          <Link href={`/${lang}/#servicios`} className="hover:opacity-80">
             {t.footer.links.services}
          </Link>
          <Link href={`/${lang}/about`} className="hover:opacity-80">
            {t.footer.links.about}
          </Link>
          <Link href={`/${lang}#contacto`} className="hover:opacity-80">
      ...
      <div className="border-t border-white/10 pt-6 flex flex-col md:flex-row justify-between items-center text-paragraph text-sm">
        ...
        {/* Redes sociales (Estas no necesitan traducción usualmente, pero podrías traducirlas si quieres) */}
        <div className="flex items-center gap-6 text-sm">
          <ButtonSecondary
            href="https://www.instagram.com/ignacioweppler/"
            target="_blank"
            className="flex text-paragraph text-sm items-center gap-1 hover:text-white transition-colors"
          >
            Instagram
          </ButtonSecondary>
